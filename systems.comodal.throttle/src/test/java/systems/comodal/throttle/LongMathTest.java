...
final class LongMathTest {
  ...
  static {
    ...
    for (final int exponent : new int[]{2, 3, 4, 9, 15, 16, 17, 24, 25, 30, 32, 33, 39, 40, 41, 47, 48, 49, 55, 56, 57}) {
    ...
    final var negativeLongCandidates = new ArrayList<Long>(ALL_LONG_CANDIDATES.size());
  ...
  private static void assertOperationEquals(final long val1, final long val2, final long expected, final long actual) {
    if (expected != actual) {
      fail("Expected for " + val1 + " s+ " + val2 + " = " + expected + ", but got " + actual);
    }
  }
