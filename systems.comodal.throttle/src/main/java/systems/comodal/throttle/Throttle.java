...
public interface Throttle {
  ...
  static Throttle create(final double permitsPerSecond, final boolean fair) {
    return new NanoThrottle(permitsPerSecond, fair);
  }
   ...
   * argument passed in the factory method that produced this {@code Throttle}.
   *
   ...
   * @throws InterruptedException     unchecked internally if thread is interrupted
   */
  void acquire(final int permits) throws InterruptedException;

  default long acquireDelayDuration() {
    return acquireDelayDuration(1);
  }
   ...
   *                                  InterruptedException and this Thread is re-interrupted
   */
  default void acquireUnchecked(final int permits) {
    try {
      acquire(permits);
